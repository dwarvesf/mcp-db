...
async function initializeDuckDB() {
    ...
    // Create connection
    connectionInstance = await new Promise((resolve, reject) => {
        const connection = new duckdb.Connection(dbInstance, (err) => {
            if (err) {
    ...
    if (databaseUrl) {
        ...
        CREATE SECRET pg_secret (
            ...
            await new Promise((resolve, reject) => {
                connectionInstance.exec(createSecretQuery, (err) => {
                    if (err) {
        ...
        ATTACH '' AS ${POSTGRES_DB_ALIAS} (
          ...
          SECRET pg_secret
            ...
            await new Promise((resolve, reject) => {
                connectionInstance.exec(attachQuery, (err) => {
                    if (err) {
...
export async function executeQueryWithRetry(query, executor) {
    let retryCount = 0;
    const maxRetries = 3;
