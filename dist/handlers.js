...
export function createToolHandlers(pgPool, duckDBConn) {
        ...
        try {
            switch (request.params.name) {
                case "duckdb_read_parquet_files": {
                    ...
                    const result = await handleDuckDBQuery(duckDBConn, request.params.arguments);
...
export function createResourceHandlers(pgPool, gcs, gcsBucket) {
    return async (request) => {
